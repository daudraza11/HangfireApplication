...
namespace Common
{
    public static class Logger
    {
            ...
            public static bool LogErrorMessage(string APIName, string FunctionName, string URL, string Data, string Headers = "", string TransID = "0", string TransRef = "0")
            {
        ...
        private static void LogError(string errorMessage, string errorType, string User, string errorSource)
        {
            try
            {
                if (HttpContext.Current != null)
                {
                    ErrorSignal.FromCurrentContext().Log(errorMessage, errorType, User, errorSource);
                }
                else
                {
                    ErrorLog errorLog = ErrorLog.GetDefault(null);
                    errorLog.Log(new Error(errorMessage, errorType, User, errorSource, null));
                }
            }
            catch (Exception)
            {
